...
//  CgalVoronoi::VD &vd = vdStatic;
  ...
  cout<<" got "<<vd.number_of_vertices() << " vertices from cgal"<<endl;
  cout<<" got "<<vd.number_of_faces() << " faces from cgal"<<endl;

//store the points' information whose distance with obstacle is larger than sConfig->voronoiMinimumDistanceToObstacle()
//contain the distance information to nearest obstacle..
...
void filterForDist(CgalVoronoi::VD &pVd, std::map<CgalVoronoi::Halfedge_handle, double> &pResult, double pMinDistance) {
    //把与点距离的平方>pMinDistance的>pMinDistance的Halfedge及其与点距离存进pResult里
